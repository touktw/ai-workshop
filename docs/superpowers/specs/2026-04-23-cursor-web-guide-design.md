...
| 파일 | 주요 내용 |
|------|-----------|
| `01.커서-가입-및-설치.md` | Cursor 홈페이지 가입 → 다운로드 → 설치 → 첫 실행 확인 |
| `02.맥-개발환경-세팅.md` | 터미널 여는 법 → Homebrew 설치 → Node.js/npm 설치 → 버전 확인 |
| `03.첫-프로젝트-만들기.md` | 프로젝트 폴더 생성 → Cursor에서 폴더 열기 → 파일 구조 소개 |
| `04.할일목록-앱-만들기.md` | Cursor AI로 할 일 목록 앱 생성 요청 → 코드 설명 → 수정해보기 |
| `05.브라우저에서-확인하기.md` | 브라우저로 index.html 열기 → Live Server 설치 → 동작 확인 |
